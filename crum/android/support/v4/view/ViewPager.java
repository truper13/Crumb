...
public class ViewPager extends ViewGroup {
    ...
    static class C00342 implements Interpolator {
        ...
        public float getInterpolation(float t) {
            t -= GalleryViewer.PROGRESS_COMPLETED;
            return ((((t * t) * t) * t) * t) + GalleryViewer.PROGRESS_COMPLETED;
        }
    }
    ...
    public static class SavedState extends BaseSavedState {
        ...
        static class C07251 implements ParcelableCompatCreatorCallbacks<SavedState> {
            C07251() {
            }
        ...
        static {
            CREATOR = ParcelableCompat.newCreator(new C07251());
        }
    ...
    void smoothScrollTo(int x, int y, int velocity) {
        ...
        float distance = ((float) halfWidth) + (((float) halfWidth) * distanceInfluenceForSnapDuration(Math.min(GalleryViewer.PROGRESS_COMPLETED, (GalleryViewer.PROGRESS_COMPLETED * ((float) Math.abs(dx))) / ((float) width))));
        velocity = Math.abs(velocity);
        if (velocity > 0) {
            duration = Math.round(1000.0f * Math.abs(distance / ((float) velocity))) * 4;
        } else {
            duration = (int) ((GalleryViewer.PROGRESS_COMPLETED + (((float) Math.abs(dx)) / (((float) this.mPageMargin) + (((float) width) * this.mAdapter.getPageWidth(this.mCurItem))))) * 100.0f);
        }
    ...
    private void calculatePageOffsets(ItemInfo curItem, int curIndex, ItemInfo oldCurInfo) {
        ...
        this.mLastOffset = curItem.position == N + INVALID_POINTER ? (curItem.offset + curItem.widthFactor) - GalleryViewer.PROGRESS_COMPLETED : AutoScrollHelper.NO_MAX;
        ...
        while (i < itemCount) {
            ...
            if (ii.position == N + INVALID_POINTER) {
                this.mLastOffset = (ii.widthFactor + offset) - GalleryViewer.PROGRESS_COMPLETED;
            }
            ii.offset = offset;
            offset += ii.widthFactor + marginOffset;
            i += SCROLL_STATE_DRAGGING;
    ...
    public void draw(Canvas canvas) {
        ...
        if (overScrollMode == 0 || (overScrollMode == SCROLL_STATE_DRAGGING && this.mAdapter != null && this.mAdapter.getCount() > SCROLL_STATE_DRAGGING)) {
            ...
            if (!this.mRightEdge.isFinished()) {
                ...
                height = (getHeight() - getPaddingTop()) - getPaddingBottom();
                canvas.rotate(90.0f);
                canvas.translate((float) (-getPaddingTop()), (-(this.mLastOffset + GalleryViewer.PROGRESS_COMPLETED)) * ((float) width));
